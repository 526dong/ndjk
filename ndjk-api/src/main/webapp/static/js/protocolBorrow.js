...
window.onload = function(){
    ...
    function ResolveURL(url,obj){
        ...
        var tempobj = decodeURI(strurl.substring(1));
        var sarr = tempobj.split("&");
        for(var i=0; i<sarr.length; i++){
            var temparr = sarr[i].split("=");
            console.log(temparr);
            if( temparr[0] == 'userID'){
                obj.usercardid = temparr[1]; //身份证号
                continue;
            }else if(  temparr[0] == 'protocalID'){
                obj.protocolid = temparr[1];//协议码
                continue;
            }else if(  temparr[0] == 'userName'){
                obj.username = temparr[1];//姓名
                continue;
            }else if(  temparr[0] == 'userTel'){
                obj.usertell = temparr[1];////电话
                continue;
            }else if(  temparr[0] == 'loanTime'){
                obj.loanTime = temparr[1];//借款时间
                continue;
            }else if(  temparr[0] == 'repayTime'){
                obj.repayTime = temparr[1];//还款时间
                continue;
            }else if(  temparr[0] == 'Allday'){
                obj.alldays = temparr[1];//借款期限
                continue;
            }else if(  temparr[0] === 'Money'){
                obj.money = temparr[1];//借款金钱
                continue;
            }
        }
        setData(obj);
